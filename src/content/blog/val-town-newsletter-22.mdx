...
Val Town is growing up! We're working on new features and infra that will fundamentally unlock the scope of what you can build in Val Town. We have two major efforts running in parallel: projects and scale.

_Projects_ are containers for multiple vals, folders, and files – enabling you to create infinitely complex real-world apps – instead of being limited to a single long val. Internal to a project, vals will use relative imports (ie `./foo/bar`), so they can be forked as a unit, and merged back in as a unit. Projects will also improve Townie's ability to help write your vals in more modular files – so it doesn't have to generate and re-generate all the code on every change.
...
As per usual, the highlight of our newsletter is at the bottom: a list of the most interesting community vals we've seen this month, including a Val Town clone built on Val Town, Stripe examples, Bluesky vals, virtual money, Cerebras demos, and almost a hundred more!
...
Our biggest effort coming up is _Projects_ - groups of vals, files, folders, forkable as a unit, mergeable as a unit, three-way-merges, relative imports

- TODO photo that got Victor inspired
- TODO https://vawogbemi-notuber.web.val.run/, https://vawogbemi-notuberfolder.web.val.run/

### Scale

We're also working on _Scale_ – a new infra architecture that will enable you to run vals in production, with scale to infinity, usage-based pricing, with usage alerts, and caps. Websocket-type vals will also launch with this effort.
