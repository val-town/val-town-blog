...
Then you may be surprised to learn that you aren't returning an object like `{ y: 1 }`, but you've defined a [labeled statement](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/label) and the function `x` will, in fact, return `undefined`. You can return an object from an arrow function, but you have to parenthesize it:
