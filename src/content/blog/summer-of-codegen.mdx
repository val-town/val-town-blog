...
First I built a basic version of code generation. We can do this (with only a few dependencies) in only 80 lines (you can fork it to play with it).
...
I use [Vercel's AI SDK](https://sdk.vercel.ai/) so we can easily switch between models. To teach the LLM how to write code on Val Town, I went for a maximalist approach: I downloaded as many public vals as I could fit in a 128k context window and put them in [`valleGetValsContextWindow`](https://www.val.town/v/janpaul123/valleGetValsContextWindow). I also found that adding docs to the context window helpful (as raw markdown pages).
...
I started with a maximalist approach of putting as much code and docs in the context window. Claude 3.5 Sonnet has a 200k token context window, I put hundreds of vals in there. However, a single query with a full context window costs at least $0.60 (just for the input tokens), which is a lot.

A full context window is also slower, and I hit rate limits much more quickly. I managed to get custom rate limits, but it's not ideal. Let's look at how I optimized our costs and speed.

### Evals

Before cutting down our context window, I wanted to measure how good our models are, so I could see how much a smaller context window would affect performance.

In the LLM world these measurements are called "evaluations" or "evals" for short (presumably because people forgot that the word "benchmark" already existed).
...
The biggest reduction we've seen in output tokens is by using diffs (instead of full regeneration), when iterating on existing vals. The longer the val is, the more it helps. We took inspiration from [aider](https://aider.chat/), which has implemented various versions of this.
...
There is a lot we'd like to experiment with, such as different diff syntaxes and semantics, including line numbers (so the model has a better point of reference), or early abort + retry if the model goes wrong.
...
Note that diffs in the UI is completely independent from whether we use full regeneration or generating diffs. It is possible in either case to immediately apply the changes to the val, or to first show diffs to the user.
...
We might get there sooner than I thought.
