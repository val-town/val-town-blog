...
Code generation ("codegen") has enabled a completely new style of building software for non-programmers, through conversation with an LLM. Millions of people are probably working on this, but some of the most successful products so far are:
...
The beauty of products like Vercel v0, Claude Projects, and Websim is that you immediately get a working program back. It's the old promise of [live coding](https://vimeo.com/906418692).

However, if you want to store data or run backend computations, that won't work. Tools like VSCode Copilot and Cursor let you build larger software with a frontend and a backend, but now you need a place to deploy it, which is a significant barrier to non-programmers, and slows the iteration cycle.

We saw many users generate their code in an external tool and then copy it into Val Town, since our "vals" (tiny full-stack webapps) get instantly deployed, and can have a **frontend, backend, and database**.

We figured we're in a unique position to tighten the feedback loop of codegen to full-stack deployment, to finally approach the dream of [end-user programming](/blog/end-programmer-programming):
...
We're now full steam ahead on productionizing this within the product, so this blog post will also give you a sneak peak at what you can expect to see soon. I really hope you enjoy.
...
<figure style={{ maxWidth: "600px" }}>
  ...
  <figcaption>
    "Make Real with Backend": one of the prototypes I made.
  </figcaption>
</figure>
