...
1. We're working with esh.sh creator to ensure this doesn't happen again.
2. Longer term, we're planning to replicate all your backend dependencies on our infrastructure, so your vals will be effectively frozen, and much more stable. We're also exploring patterns that will allow you to get more stable frontend dependencies too.
3. Shorter term, we're going to rewrite lockfile errors to be less confusing, so that the next time your val has integrity errors, it'll be much clearer what happened and what action you need to take, if anything.
