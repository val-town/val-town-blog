...
We had to entirely reorient the way we run user code to support streaming. Previously, we would run the user code and buffer the entire response, serialize it, and send it on. Now, we only communicate with user code over HTTP. If you're running a script, email, or interval Val we send an HTTP request with the relevant information and wait for a response. With HTTP vals we simply proxy the incoming request straight to the worker. This way we don't mess too much with the request structure and you'll see a request lifecycle that is very similar to running your own HTTP server.

Now that we're just talking HTTP this paves the way for future support for [websockets](https://github.com/val-town/val-town-product/discussions/144). We built this functionality in a library, and we open-sourced it! Check it out: https://github.com/val-town/deno-http-worker
