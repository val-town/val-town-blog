...
A fun way to think about TypeScript is that it does for JSON Schema what [drizzle does for SQL](https://orm.drizzle.team/) - it's a nice, TypeScript-friendly builder syntax for an existing spec that tries to expose all of the spec's power. That's not _all_ TypeBox is - the

Now, of course, there _are_ ways to use Zod like this, because the open source community is endlessly clever and persistent. For example, there's [zod-to-json-schema](https://github.com/StefanTerdell/zod-to-json-schema):
