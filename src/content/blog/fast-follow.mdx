...
Watching Windsurf take multiple actions on my behalf without my input is very inspirational. I'm dreaming of a world where Townie not only detects errors, but also automatically tries to fix them, possibly multiple times, in parallel in multiple branches, without any human interaction. Here, of course, we'd be getting into territory mostly explored by the folks at [Devin](https://devin.ai/).

For starters, we could feed back screenshots of the generated website back to the LLM, but eventually you could imagine giving it a full web browser so it can itself poke around the app, like a human would, to see what features work and which ones don't. And then write some tests, also like a human would, to make sure things don't break as it continues to iterate.
...
All this copying and how fast everything is moving begs the question: Should we get out of this race entirely and just do a better job integrating with the editors like VSCode, Cursor, Windsurf, and even Bolt? Maybe! We're planning to dip our toes into integrating: we're planning more local development support, and possibly even a [FUSE](https://en.wikipedia.org/wiki/Filesystem_in_Userspace) virtual filesystem that could make it easy to use any editors with Val Town.
