...
<figure >
  ...
  <figcaption>
    The redesigned Townie, currently in beta at <a href="https://www.val.town/townie">val.town/townie</a>.
  </figcaption>
</figure>

## Codegen

Code generation ("codegen") has enabled a completely new style of building software for non-programmers, through conversation with an LLM. 

Some of the most successful products are:

* [Claude Artifacts](
https://www.anthropic.com/news/claude-3-5-sonnet): build interactive websites and other code through conversation. Websites can be viewed directly.
...
Each Val Town user has a [SQLite](https://docs.val.town/std/sqlite/) database powered by [Turso](https://turso.tech/). We ran into a problem though: the Turso API returns data as arrays, instead of as objects. For example:
...
We have recently had [success with unified diffs](https://x.com/stevekrouse/status/1826336463543566826).
...
Our current prompt (which we be released shortly) is built around a client-side React template, which clearly demarcates between the frontend and backend parts of the code, so Claude knows it has to communicate between by making `fetch` calls on the frontend that are handled as API calls on the backend.
