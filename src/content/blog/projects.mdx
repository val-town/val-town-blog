...
import Create from "./Projects/Create.png";
...
You can break up a single val into multiple vals that import each other, but
that becomes untenable quickly. For example, say you have a couple of vals
running in production that you want to make a medium-sized change to. If you
don't want to affect production while you work, you'd have to fork each of those
vals manually. Then you'd have to manually update all the imports in each of
those vals to point to the forked versions. Then when you're ready to merge back
to main, you'd have to manually update all the imports again. This is no fun.
Instead most users either stuffed all their code into a single super long val,
just made all their changes directly to their main production vals (somtimes
breaking it briefly), or moved off Val Town entirely.
