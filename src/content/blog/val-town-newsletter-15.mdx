...
Initially we were storing logs locally and then after a while we moved them to [Cloudflare R2](https://www.cloudflare.com/developer-platform/r2/). Now we're using [ClickHouse](https://clickhouse.com/) which enabled the real-time preview functionality and makes the whole system more scalable.
...
### ⚡️ HTTP vals respond 100ms faster, and you can run code after returning a response

Till now when HTTP val returned a response, Val Town waited for a while before sending that response. The reason for that was to ensure that all promises were resolved before terminating val's execution. We introduced an optimization, where we're returning the val's HTTP response immediately, speeding up client's experience significantly.
...
We removed Val Town's biggest footgun – the Run API – without any breaking changes or downtime. For more details, check out [the announcement post](/blog/deprecating-the-run-api/).

### 📝 The first four Val Town runtimes

We have built our runtime four times! We've learned a lot about balancing safety, speed, and flexibility. [Here's the story](/blog/first-four-val-town-runtimes/).
...
We ❤️ feedback. Our feedback form emails our team with you cc'ed. Or you can:

- [make a feature request](https://github.com/val-town/val-town-product/discussions/new?category=feature-request)
- [report a bug](https://github.com/val-town/val-town-product/discussions/new?category=bug)
- [join our discord](https://discord.com/invite/dHv45uN5RY)
- [schedule a chat with our founder](https://calendly.com/steviekrouse)
- [email us](mailto:hi@val.town)
...
- https://x.com/stevekrouse/status/1755644251969355917?s=20
- https://x.com/lopesgabe_/status/1754628312906011001?s=20
